...
\begin{itemize}
    ...
    \item 
	    De makefile voorziet in de commando's \code{make commit} en \code{make push}.
	    Het commando \code[]{make commit} vraagt naar de commit message.
	    Het commando \code[]{make push} pushed de commit naar de BitBucket repository en upload de target bestanden naar de download directory van de repository. 
	    Om het commando \code{make push} te kunnen gebruiken moet de juiste waarde toegekend worden aan de variabelen \code{USER} en \code{REPOSITORY} in de makefile.
		\lstinputfragment[
		    language={make},
		    linerange={5-6}, 
		]{../makefile}
		Bij het gebruik van \code{make push} moet twee maal het wachtwoord van het betreffende BitBucket account worden ingevoerd.
		Één maal voor de push en één maal voor het oploaden naar het Download directory. 
\end{itemize}
