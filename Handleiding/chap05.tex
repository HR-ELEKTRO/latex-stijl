...
\begin{itemize}
    \item Latex kent minstens vier verschillende liggende streepjes, zie \cref{tab:streepjes}.
    ...
    \begin{itemize}
        ...
        \item Geef ISBN-nummers op met streepjes ertussen:\\ |ISBN = {978-0-201-89684-8}| om een overfull hbox te voorkomen.
            Gebruik: \url{http://tools.wmflabs.org/isbn/IsbnCheckAndFormat} om ISBN-nummers zonder streepjes om te zetten in de officiële notatie (met streepjes).
    \end{itemize}
...
\end{lstfragment}
    ...
    \item Het commando |\hurl| is een uitbreiding van het commando |\url|. 
    In de als argument meegegeven url kunnen mogelijke afbreeklocaties worden aangegeven met de tekencombinatie |\\-|.
    Dit is handig als een lange url niet correct wordt afgebroken en daardoor buiten de rechtermarge van de pagina valt.
    Het commande |\hurl| is gedefinieerd in het bestand |styleCode.tex|.
    ...
    Vergelijk selfish met self\makebox{}ish\footnote{%
    	Je moet wel inzoomen om het verschil te kunnen zien. In selfish zit de f vast aan de i en bij het selecteren van tekst is de fi één letter. In self\makebox{}ish zit een beetje ruimte tussen de f en de i en bij het selecteren van tekst zijn het twee aparte letters.  
	}.
    ...
    \item Soms kom je net een beetje ruimte op een pagina tekort.
    Je kunt dan |\enlargethispage{.5\baselineskip}| gebruiken om de pagina met een halve regel te verlengen.
\end{itemize}
...
\begin{itemize}
    \item 
        ...
        Binnen de opleiding Elektrotechniek van de Hogeschool Rotterdam maken we gebruik van private repositories op BitBucket (zie: \url{https://bitbucket.org/HR_ELEKTRO/}).
        We maken gebruik van BitBucket voor onze private repositories en niet van GitHub omdat de eerste gratis private repositories biedt.
    \item 
        De gecompileerde pdf-documenten worden gepubliceerd op een publieke repository op GitHub (zie: \url{https://github.com/HR-ELEKTRO/}),op Brighspace (zie: \url{https://brightspace.hr.nl/}) of in de Downloads van een BitBucket repository (zie bijvoorbeeld: \url{https://bitbucket.org/HR_ELEKTRO/latex-stijl/downloads/}).
        We maken gebruik van GitHub voor onze wiki's omdat BitBucket sinds augustus 2026 geen wiki's meer ondersteunt.
    \item 
