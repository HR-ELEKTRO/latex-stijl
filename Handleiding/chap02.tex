...
% #1 = nummer van deel, bijvoorbeeld 6.2
% #2 = deel van de link achter # (nameddest=#1.#2 werkt niet meer)
\makeatletter
\newcommand{\lshortdeel}[2]{%
    \@saveurlcite{lshort}%
    \edef\@savedurldeel{\@savedurl\##2}%
    \href{\@savedurldeel}{#1}%
}
% #1 = naam van het deel b.v. hoofdstuk
% #2 = nummer van deel, bijvoorbeeld 6.2
% #3 = deel van de link achter # (nameddest=#1.#2 werkt niet meer)
\newcommand{\lshortcitedeel}[3]{%
	\@saveurlcite{lshort}%
	\edef\@savedurldeel{\@savedurl\##3}%
	\cite[\href{\@savedurldeel}{#1~#2}]{lshort}%
}
\makeatother

\newcommand{\lshortcitechapter}[2]{\lshortcitedeel{hoofdstuk}{#1}{#2}}
\newcommand{\lshortcitesection}[2]{\lshortcitedeel{paragraaf}{#1}{#2}}
...
Als je haast hebt, kun je volstaan met het lezen van paragrafen \lshortdeel{1.3}{a}, \lshortdeel{1.8}{16}, \lshortdeel{2.1}{18}, \lshortdeel{2.6}{29}, \lshortdeel{2.7}{2a}, \lshortdeel{2.9}{2c},
\lshortdeel{2.10}{2d}, \lshortdeel{2.11.1}{2f} en \lshortdeel{2.11.3}{31}.
...
Bijvoorbeeld: \lshortcitesection{2.4.1}{1e} kan worden vervangen door \cref{sec:aanhalingstekens},
\lshortcitesection{2.4.9}{26} is overbodig vanwege de in deze stijl gebruikte karakter encoding,
zie \csecref{sec:karakterencoding}, \lshortcitesection{2.8}{2b} wordt \squote{overruled} door \csecref{sec:verwijzingen},
\lshortcitesection{2.11.5}{33} wordt vervangen door \csecref{sec:listings}, \lshortcitesection{2.11.6}{34} wordt vervangen door \csecref{sec:tabellen} en
\lshortcitesection{2.12}{35} wordt vervangen door \csecref{sec:figuren,sec:tabellen}.

Als je een document gaat maken waar wiskundige formules in voorkomen dan raad ik je aan om ook \lshortcitechapter{3}{37} te lezen.
...
De in paragrafen~\lshortdeel{3.5.2}{3f} en \lshortdeel{3.5.3}{40} van \cite{lshort} beschreven package |IEEEtrantools| moet je,
...
De papieren versie is bedoeld om dubbelzijdig te printen, heeft een bindmarge, hoofdstukken beginnen op oneven pagina's%
% AANGEPAST 01-01-2023 en links hebben geen kleur (klikken op papier werkt namelijk niet),
%maar werken wel (voor het geval iemand toch liever deze versie elektronisch leest)
.
De ebook versie is enkelzijdig%
% en heeft gekleurde links
.
...
\footnote{%
	Zie: \href{https://github.com/Harry-Broeders/tds02/wiki/Report_Requirements/Report_Requirements_TDS02.pdf}{Report\_Requirements\_TDS02.pdf} en 
	\href{https://github.com/Harry-Broeders/tds02/wiki/Report_Requirements/Report_Requirements_TDS02_ebook.pdf}{Report\_Requirements\_TDS02\_ebook.pdf}.
	De sourcecode vind je hier: \url{https://bitbucket.org/HR_ELEKTRO/tds02/src/master/Report\%20Requirements}.%
}.

Als je de commando's |\frontmatter| en |\mainmatter| gebruikt in een book, dan worden de pagina's tussen deze twee commando's (in de papieren versie) Romeins genummerd.
...
De in dit document beschreven stijl is ook te gebruiken om een cursushandleiding mee te maken. Er is daarvoor een speciale file |styleCursusHandleiding.tex| beschikbaar. Je kunt de cursushandleiding van EMS3\footnote{%
	Zie \url{https://github.com/HR-ELEKTRO/ems3/wiki}, de source is te vinden in de private repository van EMS3. 
...
\section{Versiehistoriezondernamen}
\label{sec:versiehistoriezondernamen}
Als je om een of andere reden, bijvoorbeeld vanwege privacy-overwegingen, de namen van de auteurs niet in de versiehistorie wilt opnemen, dan kun je gebruik maken van het commando |\versiehistoriezondernamen|.
Dit commando heeft dezelfde syntax als het commando |\versiehistorie|, zie \cref{sec:versiehistorie}, maar heeft slechts 3 kolommen: Datum, Versie en Omschrijving.
...
Als je een woord tussen dubbele aanhalingstekens wilt plaatsen, dan kun je dat, zoals je vast wel weet, in \LaTeX{} niet doen door het teken |"| te gebruiken omdat dit teken gebruikt
...
In plaats van het commando |\figuur| kun je ook het commando |\hrfigure| gebruiken. Dit commando heeft slechts twee parameters waarbij de eerste optioneel is. 
...
De code: |\hrfigure[width=2cm, label=klzebra, caption=Kleine zebra.]{zebra}| produceert \cref{klzebra}.

\hrfigure[width=2cm, label=klzebra, caption=Kleine zebra.]{zebra}
...
Het commando |\tikzfiguur| kan gebruikt worden om een |tikzpicture|-environment die in een apart document is geplaatst in een document op te nemen. 
...
In plaats van het commando |\tikzfiguur| kun je ook het commando |\hrtikzfigure|\label{hrtikzfigure} gebruiken. Dit commando heeft slechts twee parameters waarbij de eerste optioneel is. 
De tweede parameter geeft de filenaam (zonder extensie).
Dit bestand moet in het directory |./tikz| geplaatst zijn.
...
Als je de Ti$k$Z-code liever in het document zelf opneemt, dan kun je het commando {\lstlcs\hcode{\\tikz\\-inline\\-fi\\-guur}} gebruiken.
Dit commando heeft vier parameters waarvan de eerste optioneel is.
De eerste parameter bevat de \squote{placement} argumenten voor de |figure|-environment.
De tweede parameter is het suffix van het label ({\lstllabel\code{fig:suffix}}).
...
In plaats van het commando |\tikzinlinefiguur| kun je ook het commando {\lstlcs\hcode{\\hr\\-tikz\\-in\\-line\\-figure}} gebruiken.
...
\subsection{Meerdere figuren of andere dingen naast elkaar}
\label{sec:meerderefiguren}
Als je gebruik maakt van één van de bovenstaande commando's om figuren in een document op te nemen, dan staat er niets links of rechts van het figuur, zie \cref{klzebra}.
Soms is het handiger om meerdere kleine figuren (of andere dingen) naast elkaar te plaatsen.
Dit kan met behulp van de |multicols|-environment van de package |multicol|\footnote{\url{https://www.ctan.org/pkg/multicol}}.
Deze environment heeft één optionele parameter waarin je kunt opgeven hoeveel kolommen je wilt hebben.

De code die gegeven is in \cref{lst:multicols} zet \cref{fig:teddybeer1,fig:teddybeer2} naast elkaar.

\floatlstinput[
    style=lstyle,
    rangebeginprefix= \%>,
    rangeendprefix=\%<,
    linerange={beren-beren},
]{../chap02.tex}{multicols}{\LaTeX{}-code waarmee \cref{fig:teddybeer1} en \cref{fig:teddybeer2} naast elkaar gezet zijn.}

%>beren
\begin{multicols}{2}
	\figuur[H]{width=.8\columnwidth}{teddybeer1}{Plaatje van een teddybeer.}
	\columnbreak
	\figuur[H]{width=.8\columnwidth}{teddybeer2}{Plaatje van een andere teddybeer.}
\end{multicols}
%<beren

De code die gegeven is in \cref{lst:beerformule} zet \cref{fig:teddybeer3} en \cref{eq:life} naast elkaar.

\lstinput[
    style=lstyle,
    rangebeginprefix= \%>,
    rangeendprefix=\%<,
    linerange={beerformule-beerformule},
]{../chap02.tex}{beerformule}{\LaTeX{}-code waarmee \cref{fig:teddybeer3} en \cref{eq:life} naast elkaar gezet zijn.}

%>beerformule
\begin{multicols}{2}
	\hrfigure[placement=H, width=.8\columnwidth, label=fig:teddybeer3, caption=Teddybeer met T-shirt.]{teddybeer1}
	\columnbreak
	\vspace*{1cm}
	\begin{equation}\label{eq:life}
		\int\limits_{birth}^{death} struggle\;\;dt=life
	\end{equation}
\end{multicols}
%<beerformule
...
\section{Tabellen}
\label{sec:tabellen}
Voor tabellen werd in eerdere versies van deze style de package |tabu|\footnote{\url{https://www.ctan.org/pkg/tabu}} gebruikt.
...
Voor normale tabellen die gebruik maken van |tabular| kunnen de commando's:
...
Als je tabellen wilt maken waarbij de tekst in een kolom uitgevuld wordt, dan moet je gebruik maken van commando's die gebaseerd zijn op |tabularx|. 
Dit zijn de commando's:
|\tabelX| en |\langetabelX|.

Deze commando's gebruiken normaal gesproken de volledige tekstbreedte.
Je kunt als eerste optionele parameter ook de gewenste breedte opgeven.
Bij deze commando's kun je gebruik maken van de kolomspecificatie |X|. 
...
%>tabelvoorbeeld2
\tabelX{Zomaar een uitgevulde tabel.}{gebruikX}{XX}{
	Kolom 1 & Kolom2
}{        
...
De code die gegeven is in \cref{lst:tabelvoorbeeld3} produceert \cref{tab:gebruikXongelijk}.
...
]{../chap02.tex}{tabelvoorbeeld3}{\LaTeX{}-code waarmee \cref{tab:gebruikXongelijk} gemaakt is.}

%>tabelvoorbeeld3
\tabelX{Zomaar een uitgevulde tabel met kolommen van ongelijke breedte.}{gebruikXongelijk}{ >{\hsize=0.67\hsize\linewidth=\hsize}X >{\hsize=1.33\hsize\linewidth=\hsize}X}{
	Kolom 1 & Kolom2
}{        
	Zomaar een stukje tekst om te laten zien hoe deze tekst uitgevuld en uitgelijnd wordt.&
	Zomaar een stukje tekst om te laten zien hoe deze tekst uitgevuld en uitgelijnd wordt. 
...
\tabel{Verschillende prefixen van de \lcode{X}-kolomspecifier.}{xspecs}{ll}{
...
De code die gegeven is in \cref{lst:tabelvoorbeeld4} produceert \cref{tab:gebruikXspecs}.

\floatlstinput[
style=lstyle, 
rangebeginprefix= \%>,
rangeendprefix=\%<,
linerange={tabelvoorbeeld4-tabelvoorbeeld4},
]{../chap02.tex}{tabelvoorbeeld4}{\LaTeX{}-code waarmee \cref{tab:gebruikXspecs} gemaakt is.}

%>tabelvoorbeeld4
\tabelX{Tabel met X kolommen.}{gebruikXspecs}{>{\centering}X >{\raggedleft}X >{\raggedright}XX}{
...
Zoals je in \cref{tab:gebruikX} kunt zien worden de regels verticaal aan de top van de regel uitgelijnd.
...
De code die gegeven is in \cref{lst:tabelvoorbeeld5} produceert \cref{tab:gebruikXm}.

\floatlstinput[
style=lstyle, 
rangebeginprefix= \%>,
rangeendprefix=\%<,
linerange={tabelvoorbeeld5-tabelvoorbeeld5},
]{../chap02.tex}{tabelvoorbeeld5}{\LaTeX{}-code waarmee \cref{tab:gebruikXm} gemaakt is.}

%>tabelvoorbeeld5
\tabelXm{Zomaar een uitgevulde tabel waarbij de regels verticaal in het midden uitgelijnd worden}{gebruikXm}{XX}{
	Kolom 1 & Kolom2
}{        
	Zomaar een stukje tekst om te laten zien hoe deze tekst uitgevuld en uitgelijnd wordt.&
	Zomaar een stukje tekst om te laten zien hoe deze tekst uitgevuld en uitgelijnd wordt. 
...
\pdfdeelref{section}{paragraaf~}{https://github.com/HR-ELEKTRO/cppprog/wiki/Dictaat_OPinCpp\iftoggle{ebook}{_ebook}{}.pdf}{6.2}
naar paragraaf 6.2 van mijn \Cpp{}-dictaat.
De voorgaande link is als volgt gecodeerd:

\begin{lstfragment}[style=lstyle]
\pdfdeelref{section}{paragraaf~}{%
https://github.com/HR-ELEKTRO/cppprog/wiki/Dictaat_OPinCpp%
...
https://github.com/HR-ELEKTRO/dis1/wiki/Dictaat/Dictaat_DIS10%
\iftoggle{ebook}{_ebook}{}.pdf}{4} van het DIS10 dictaat.
\end{lstfragment}
Levert de volgende uitvoer:
Bla, bla, zie
\pdfdeelref{chapter}{hoofdstuk~}{https://github.com/HR-ELEKTRO/dis1/wiki/Dictaat/Dictaat_DIS10\iftoggle{ebook}{_ebook}{}.pdf}{4}
van het DIS10 dictaat.
...
https://github.com/HR-ELEKTRO/cppprog/wiki/Dictaat_OPinCpp%
\iftoggle{ebook}{_ebook}{}.pdf}{15.4}]{Broeders2019}.
\end{lstfragment}

Bovenstaande code geeft: Bla, bla namespaces,
zie \cite[\pdfdeelref{section}{paragraaf~}{https://github.com/HR-ELEKTRO/cppprog/wiki/Dictaat_OPinCpp\iftoggle{ebook}{_ebook}{}.pdf}{15.4}]{Broeders2019}.
...
\begin{lstfragment}[style=lstyle]
Bla, bla namespaces, zie \citepdfdeel{section}{paragraaf~}{Broeders2019}{15.4}.
\end{lstfragment}
...
De code:
\begin{lstfragment}[style=lstyle]
Bla, bla, zie \pdfpagref[14]{%
https://ftp.snt.utwente.nl/pub/software/tex/info/lshort/ english/lshort.pdf}%
{85} van \emph{The Not So Short Introduction to \LaTeXe}.
\end{lstfragment}
Levert de volgende uitvoer:
Bla, bla, zie \pdfpagref[14]{%
https://ftp.snt.utwente.nl/pub/software/tex/info/lshort/english/lshort.pdf}%
...
In de bovenstaande code wordt verwezen de pagina met paginanummer 85 maar dat is in werkelijkheid de $99^{\text{ste}}$ pagina in het document
vandaar dat als eerste parameter $99-85=14$ wordt meegegeven.
...
%Verie 2.4: Documentatatie voor \handtekening verwijderd.